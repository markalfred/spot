...
setContentRows = function(rows) {
  ...
  rows = _.map(content.collection, function(c) {
    return [c.name.substr(0, maxWidth), _.map(c.artists, 'name').join(', ').substr(0, maxWidth), c.album.name.substr(0, maxWidth), moment(c.duration_ms).format('m:ss').substr(0, maxWidth)];
  });
